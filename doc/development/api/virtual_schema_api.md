...
* `pushdownRequest`: Specification what needs to be pushed down. You can think of it like a parsed SQL statement.
  ...
  * `selectList`: The requested select list. There are three options for this field:
    ...
    * `selectListDataTypes`: List of data types expected in the result set. This information is only provided by Exasol databases with version 8.6.0 and higher or major version 7 and version &ge; 7.1.14, see [Exasol Data Types API Documentation](data_types_api.md). The list of data types passed on are those stored in the virtual schema's virtual table definitions when creating or refreshing the virtual schema.
