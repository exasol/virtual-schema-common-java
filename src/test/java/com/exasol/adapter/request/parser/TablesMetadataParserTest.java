...
class TablesMetadataParserTest {
    ...
    void testParseMetadata() throws IOException, AdapterException {
        ...
        tableColumns.add(ColumnMetadata.builder().name("URL").adapterNotes("").type(DataType.createVarChar(1000, UTF8))
              .nullable(true).identity(true).defaultValue("").comment("").build());
        tableColumns
              .add(ColumnMetadata.builder().name("REQUEST_TIME").adapterNotes("").type(DataType.createTimestamp(false))
                    .nullable(true).identity(true).defaultValue("").comment("").build());
    ...
    void testParseTablesMetadataAllColumnsTypes() throws IOException {
        ...
        final JsonArray tablesAsJson =
              readInvolvedTablesFromJsonFile("target/test-classes/pushdown_request_alltypes.json");
        final TablesMetadataParser tablesMetadataParser = new TablesMetadataParser();
        final List<TableMetadata> tables = tablesMetadataParser.parse(tablesAsJson);
        assertThat(tables, equalTo(expectedInvolvedTablesMetadata));
    }
    ...
    private List<TableMetadata> createExpectedTableMetadata() {
        ...
        tableColumns.add(ColumnMetadata.builder().name("C_VARCHAR_ASCII").adapterNotes("")
              ...
              .build());
        tableColumns
              .add(ColumnMetadata.builder().name("C_CHAR_UTF8_1").adapterNotes("").type(DataType.createChar(3, UTF8))
                    .nullable(true).identity(true).defaultValue("").comment("").build());
        tableColumns
              .add(ColumnMetadata.builder().name("C_CHAR_UTF8_2").adapterNotes("").type(DataType.createChar(3, UTF8))
                    .nullable(true).identity(true).defaultValue("").comment("").build());
        tableColumns
              .add(ColumnMetadata.builder().name("C_CHAR_ASCII").adapterNotes("").type(DataType.createChar(3, ASCII))
                    .nullable(true).identity(true).defaultValue("").comment("").build());
        tableColumns
              .add(ColumnMetadata.builder().name("C_DATE").adapterNotes("").type(DataType.createDate()).nullable(true)
                    .identity(true).defaultValue("").comment("").build());
        tableColumns
              .add(ColumnMetadata.builder().name("C_TIMESTAMP_1").adapterNotes("").type(DataType.createTimestamp(false))
                    .nullable(true).identity(true).defaultValue("").comment("").build());
        tableColumns
              .add(ColumnMetadata.builder().name("C_TIMESTAMP_2").adapterNotes("").type(DataType.createTimestamp(false))
                    .nullable(true).identity(true).defaultValue("").comment("").build());
        tableColumns
              .add(ColumnMetadata.builder().name("C_TIMESTAMP_3").adapterNotes("").type(DataType.createTimestamp(true))
                    .nullable(true).identity(true).defaultValue("").comment("").build());
